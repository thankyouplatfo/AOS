...
@section('content')
    ...
    <header class="w3-container" style="padding-top:22px">
        <h5 class="w3-xxlarge"><b class="w3-right-align"><i class="fa fa-university" aria-hidden="true"></i> إضافة جامعة </b><a
                href="{{  route('admin.addUniversity.create') }}" class="w3-left"><i class="fa fa-plus"></i></a></h5>
    </header>
        ...
        <div class="w3-row-padding" style="margin:0 -16px">
            ...
            <div class="w3-responsive">
                ...
                <table id="id01" class="w3-table w3-striped w3-white w3-centered w3-bordered">
                    ...
                    @foreach ($addUniversity as $addUniversity)
                        <tr class="item">
                            <td>
                                {{ $addUniversity->id }}
                            </td>
                            <td id="td-bg" style="background-image: url({{ asset('images/addUniversity/'.$addUniversity->image) }})">
                            ...
                            <td>
                                {{ $addUniversity->addCountry->name }}
                            </td>
                            <td>
                                {{ $addUniversity->addCity->name }}
                            </td>
                            <td>
                                @php if($addUniversity->type == 1){ echo 'حكومي';}elseif($addUniversity->type == 2){ echo 'أهلي';}elseif($addUniversity->type == 3){ echo 'أخرى';} @endphp
                            </td>
                            <td>
                                {{ $addUniversity->internationalRanking }}
                            </td>
                            <td>
                                {{ $addUniversity->localRanking }}
                            </td>
                            <td>
                                <a href="{{ $addUniversity->url }}" target="_blank">انقر للزيارة</a>
                            </td>
                            <td>
                                <details>
                                    <summary>انقر للإطلاع</summary>
                                    <p>{{$addUniversity->about}}.</p>
                                  </details>
                            </td>
                            <td>
                                <a href="{{ route('university',$addUniversity->id) }}"
                                    class=" w3-hover-none w3-button w3-bar-item" target="_blank"><i
                                        class="w3-xlarge fa fa-eye cnsb-hov-txt-green"></i></a>
                            </td>
                            <td>
                                <a href="{{ route('admin.addUniversity.edit', $addUniversity) }}"
                                    class=" w3-hover-none w3-button w3-bar-item"><i
                                        class="w3-xlarge fa fa-edit cnsb-hov-txt-blue"></i></a>
                            </td>
                            <td>
                                <form action="{{ route('admin.addUniversity.destroy', $addUniversity) }}" method="post">@csrf<button
