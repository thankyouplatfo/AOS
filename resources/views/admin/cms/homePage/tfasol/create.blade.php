...
<div class="w3-container w3-large">
    ...
    <div class="w3-bar" style="margin-top: 44px">
        <div class="w3-bar-item w3-right">
            <h1 class="fa-2x" style="padding: 0px;margin:0px;" >تحميل ملف التفاضل <span id="main-title" style="padding: 0px;margin:0px;font-family:'Aref Ruqaa', serif!important;"></span></h1>
        </div>
    </div>
    
    <form action="{{ route('admin.cms.homePage.tfasol.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <p>
        ...
        <p>
            <label for="title">عنوان | <span class="w3-opacity-max">صف محتويات الملف 72-881 حرف</span></label>
            <textarea  class="w3-input title" name="title" id="title" minlength="3" maxlength="881" cols="30" rows="10">{{old('title')}}</textarea>
            @error('title')
                {{$message}}
            @enderror
        </p>
