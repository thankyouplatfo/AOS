...
@section('content')
    ...
    <header class="w3-container" style="padding-top:22px">
        <h5 class="w3-xxlarge"><b><i class="fa fa-faile" aria-hidden="true"></i> التفاضل </b><a
                href="{{ route('admin.cms.homePage.tfasol.create') }}" class="w3-left"><i class="fa fa-plus"></i></a></h5>
    </header>
        ...
        <div class="w3-row-padding" style="margin:0 -16px">
            ...
            <div class="w3-responsive">
                ...
                <table id="id01" class="w3-table w3-striped w3-white w3-centered w3-bordered">
                    ...
                    @foreach ($tfasol as $tfasol)
                        <tr class="item">
                            <td>
                                {{ $tfasol->id }}
                            </td>
                            <td>
                                <a href="{{asset('file/tfasol/'.$tfasol->file)}}" title="{{$tfasol->title}}">انقر للإطلاع</a>
                            </td> 
                            <td>
                                @switch($tfasol->openClose)
                                    @case(1)
                                        {{'مفتوح'}}
                                        @break
                                    @case(2)
                            ...
                            <td>
                                <a href="{{ route('home') }}"
                                    class=" w3-hover-none w3-button w3-bar-item" target="_blank"><i
                                        class="w3-xlarge fa fa-eye cnsb-hov-txt-green"> انقر على زر التفاضل </i></a>
                            </td>
                            <td>
                                <a href="{{ route('admin.cms.homePage.tfasol.edit', $tfasol) }}"
                                    class=" w3-hover-none w3-button w3-bar-item"><i
                                        class="w3-xlarge fa fa-edit cnsb-hov-txt-blue"></i></a>
                            </td>
                            <td>
                                <form action="{{ route('admin.cms.homePage.tfasol.destroy', $tfasol) }}" method="post">@csrf<button
