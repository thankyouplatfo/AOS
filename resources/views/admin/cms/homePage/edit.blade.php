...
<div class="w3-container w3-large">
    ...
    <div class="w3-bar" style="margin-top: 44px">
        <div class="w3-bar-item w3-right">
            <h1 class="fa-2x" style="padding: 0px;margin:0px;" >حرر عدل على الصفحة الرئيسية <span id="main-title" style="padding: 0px;margin:0px;font-family: 'Aref Ruqaa', serif!important;"></span></h1>
        </div>
    </div>
     <form action="{{ route('admin.cms.homePage.update',$CMSHomePage) }}" method="POST" enctype="multipart/form-data">
        
        @csrf
        <p>
            <label for="headerImage">الصورة الرئيسية </label>
            <input accept="image/*" type="file" value="{{$CMSHomePage->headerImage}}" class="w3-input headerImage" name="headerImage" id="headerImage">
            @error('headerImage')
                {{ $message }}
            @enderror
        </p>
        <p>
            ...
            <input type="text" minlength="3" maxlength="881" value="{{$CMSHomePage->altHeaderImage}}" class="w3-input altHeaderImage" name="altHeaderImage" id="altHeaderImage">
        ...
        <p>
            ...
            <textarea type="text" class="w3-input about" name="about" id="about" cols="30" rows="5" minlength="72" maxlength="812">{{$CMSHomePage->about}}</textarea>
            @error('about')
                {{ $message }}
            @enderror
        </p>
        <p>
            ...
            <textarea type="text" value="" class="w3-input keywords" name="keywords" id="keywords" cols="30" rows="5" minlength="72" maxlength="812">{{$CMSHomePage->keywords}}</textarea>
            @error('keywords')
                {{ $message }}
            @enderror
        ...
        <p>
            ...
            <input type="text" value="{{$CMSHomePage->address}}" class="w3-input address" name="address" id="address" minlength="3" maxlength="72">
            @error('address')
                {{ $message }}
            @enderror
        </p>
        <p>
            ...
            <input type="tel" dir="ltr" maxlength="25" minlength="7" value="{{$CMSHomePage->tel}}" class="w3-input tel w3-left-align  tel" name="tel" id="tel">
            @error('tel')
                {{ $message }}
            @enderror
        </p>
        <p>
            ...
            <input type="email" dir="ltr" maxlength="25" minlength="7" value="{{$CMSHomePage->email}}" class="w3-input email w3-left-align  mail" name="email" id="email">
            @error('email')
                {{ $message }}
            @enderror
        </p>
        <p>
            ...
            <input accept="image/*" type="file" value="{{$CMSHomePage->mapImage}}" class="w3-input mapImage" name="mapImage" id="mapImage">
            @error('mapImage')
                {{ $message }}
            @enderror
        </p>
        <p>
            ...
            <input type="text" minlength="3" maxlength="881" value="{{$CMSHomePage->altMapImage}}" class="w3-input altMapImage" name="altMapImage" id="altMapImage">
            @error('altMapImage')
                {{ $message }}
            @enderror
            <span class="cnsb-txt-red"> مهم لمحركات البحث والتسويق الاكتروني</span>
        ...
        <p>
            ...
            <input value="{{$CMSHomePage->whatsAppID}}" class="w3-input whatsAppID" name="whatsAppID" id="whatsAppID">
            @error('whatsAppID')
                {{ $message }}
            @enderror
        </p>
