...
<div class="w3-container w3-large">
    ...
    <div class="w3-bar" style="margin-top: 44px">
        <div class="w3-bar-item w3-right">
            <h1 class="fa-2x" style="padding: 0px;margin:0px;" >حرر عدل على ارتباط موقع تواصل اجتماعي <span id="main-title" style="padding: 0px;margin:0px;font-family: 'Aref Ruqaa', serif!important;"></span></h1>
        </div>
    </div>
     <form action="{{ route('admin.cms.social.update',$social) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <p>
            <label for="url">الرابط <span class="w3-medium cnsb-txt-red">رابط صفحتك على موقع التواصل الاجتماعي</span></label> 
            <input type="text" class="w3-input url" name="url" id="url" minlength="3" maxlength="255" value="{{ $social->url }}">
            @error('url')
                {{ $message }}
            @enderror
        </p>
		<p>
            ...
            <input type="text" class="w3-input title" name="title" id="title" minlength="3" maxlength="255" value="{{ $social->title }}" placeholder="صفحتنا على فيسبوك">
            @error('title')
                {{ $message }}
            @enderror
        </p>
		<p>
            ...
            <input type="text" class="w3-input icon ltr" name="icon" id="icon" minlength="3" maxlength="255" value="{{ $social->icon }}" placeholder="fa fa-facebook">
            @error('icon')
                {{ $message }}
            @enderror
        </p>
