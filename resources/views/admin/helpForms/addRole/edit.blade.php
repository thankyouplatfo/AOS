...
<div class="w3-container w3-large">
    ...
    <div class="w3-bar" style="margin-top: 44px">
        <div class="w3-bar-item w3-right">
            <h1 class="fa-2x" style="padding: 0px;margin:0px;" > حرر عدل على دور 
                <span style="padding: 0px;margin:0px;font-family: 'Aref Ruqaa', serif!important;">{{$addRole->name}}</span></h1>
        </div>
    </div>
    <form action="{{ route('admin.helpForms.addRole.update',$addRole) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <p>
        ...
        <p>
            ...
            <input type="hidden" class="w3-input slug" name="slug" value="{{$addRole->slug}}" id="slug">
            @error('slug')
                {{ $message }}
            @enderror
        </p>
