...
<div class="w3-container w3-large">
    ...
    <div class="w3-bar" style="margin-top: 44px">
        <div class="w3-bar-item w3-right">
            <h1 class="fa-2x" style="padding: 0px;margin:0px;" >حرر عدل على  فئة أداة <span id="main-title" style="padding: 0px;margin:0px;font-family: 'Aref Ruqaa', serif!important;"></span></h1>
        </div>
    </div>
     <form action="{{ route('admin.helpForms.addCategoryTool.update',$addCategoryTool) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <p>
            <label for="name">الاسم</label>
            <input type="text" name="name" class="w3-input name" id="name" value="{{$addCategoryTool->name}}" minlength="3" maxlength="40">
            @error('name')
                {{$message}}
            @enderror
        </p>
        <p>
            ...
            <input type="hidden" name="slug" class="w3-input slug" id="slug" value="{{$addCategoryTool->slug}}" minlength="3" maxlength="40">
            @error('slug')
                {{$message}}
            @enderror
        </p>
