    ...
    <div class="w3-container w3-large">
        ...
        <div class="w3-bar" style="margin-top: 44px">
            <div class="w3-bar-item w3-right">
                <h1 class="fa-2x" style="padding: 0px;margin:0px;"> أضف تخصص جديد باسم <span id="main-title"
        ...
        <form action="{{ route('admin.addSpecialty.store') }}" method="POST" enctype="multipart/form-data">
            @csrf    
            <p>
                <label for="image">الصورة</label>
                <input type="file" class="w3-input image" name="image" id="image" minlength="3" max="881" value="{{old('image')}}" accept="image/*"> 
                @error('image')
                    {{$message}}
                @enderror  
            </p>
            <p>
                ...
                <input type="text" class="w3-input alt" name="alt" id="alt" minlength="3" max="881" value="{{old('alt')}}"> 
                @error('alt')
                    {{$message}}
                @enderror  
            </p>
            <p>
                ...
                <input type="text" class="w3-input name" name="name" id="name" minlength="3" max="881" value=" تخصص {{old('name')}}" oninput="$('#slug').val($('#name').val().replace(/\s+|_+/g,'-').toLowerCase()); $('#main-title').text($('#name').val())"> 
                @error('name')
                    {{$message}}
                @enderror  
            </p>
            <p>
                ...
                <input type="hidden" class="w3-input slug" name="slug" id="slug" minlength="3" max="881" value="{{old('slug')}}"> 
                @error('slug')
                    {{$message}}
                @enderror  
            </p>
            <p>
                ...
                <input type="text" class="w3-input url" name="url" id="url" minlength="3" max="881" value="{{old('url')}}"> 
                @error('url')
                    {{$message}}
                @enderror  
            </p>
            ...
            <p>
                ...
                <textarea class="w3-input about" name="about" id="about" minlength="3" max="881" cols="30" rows="10">{{old('about')}}</textarea>
                @error('about')
                    {{$message}}
                @enderror  
            </p>
            <p>
                ...
                <input type="text" class="w3-input keywords" name="keywords" id="keywords" minlength="3" max="881" value="{{old('keywords')}}"> 
                @error('keywords')
                    {{$message}}
                @enderror  
            </p>
