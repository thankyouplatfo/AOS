...
@section('content')
    ...
    <header class="w3-container" style="padding-top:22px">
        <h5 class="w3-xxlarge"><b><i class="fa fa-list" aria-hidden="true"></i> فئات للأدوات </b><a
                href="{{ route('admin.addTool.create') }}" class="w3-left"><i class="fa fa-plus"></i></a></h5>
    </header>
        ...
        <div class="w3-row-padding" style="margin:0 -16px">
            ...
            <div class="w3-responsive">
                ...
                <table id="id01" class="w3-table w3-striped w3-white w3-centered w3-bordered">
                    ...
                    @foreach ($addTool as $addTool)
                        <tr class="item">
                            <td>
                                {{ $addTool->id }}
                            </td>
                            <td>
                                <a href="{{$addTool->url}}" target="_blank">{{$addTool->name}}</a>
                            </td>
                            <td>
                                {{$addTool->addCategoryTool->name}}
                            </td>
                            <td>
                                <details>
                            ...
                            <td>
                                <a type="disabled" href="{{ route('admin.addTool.edit', $addTool) }}"
                                    class=" w3-hover-none w3-button w3-bar-item"><i
                                        class="w3-xlarge fa fa-edit cnsb-hov-txt-blue"></i></a>
                            </td>
                            <td>
                                <form action="{{ route('admin.addTool.destroy', $addTool) }}" method="post">@csrf<button
