...
<div class="w3-container w3-large">
    ...
    <div class="w3-bar" style="margin-top: 44px">
        <div class="w3-bar-item w3-right">
            <h1 class="fa-2x" style="padding: 0px;margin:0px;" >عدل على معلومات أداة باسم <span id="main-title" style="padding: 0px;margin:0px;font-family:'Aref Ruqaa', serif!important;"></span></h1>
        </div>
    </div>
    
    <form action="{{ route('admin.addTool.update',$addTool) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <p>
            <label for="name">الاسم</label>
            <input type="text" class="w3-input name" name="name" id="name" minlength="3" max="40" value="{{$addTool->name}}">
            @error('name')
                {{$message}}
            @enderror
        </p>
        <p>
            ...
            <input type="text" class="w3-input url" name="url" id="url" minlength="3" max="40" value="{{$addTool->url}}">
            @error('url')
                {{$message}}
            @enderror
        </p>
        <p>
            ...
            <textarea class="w3-input about" name="about" id="about" minlength="3" maxlength="881" cols="30" rows="10">{{$addTool->about}}</textarea>
            @error('about')
                {{$message}}
            @enderror
        </p>
